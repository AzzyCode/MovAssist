...
classifier_model = "squat_form_classifier.h5"

current_dir = os.getcwd()
src_dir = os.path.dirname(current_dir)
project_root = os.path.dirname(src_dir)
model_path = os.path.join(os.path.expanduser("~"), "Desktop", "MovAssist", "models", classifier_model)
...
class BaseExercise:
    ...
    def __init__(self, min_detecting_confidentce=0.5, min_tracking_confidence=0.5, use_camera=False):
        ...
        self.buffer_size = 30
        self.frame_buffer = np.zeros((self.buffer_size, 132), dtype=np.float32)
        ...
        self.prediction_thread = Thread(target=self.start_prediction_loop, daemon=True)
    ...
    def process_rep_frames(self):
        """Collect frames for a single rep."""
        if len(self.rep_frame_sequence) < self.buffer_size:
            logging.warning(f"Insufficient frames: {len(self.rep_frame_sequence)}/{self.buffer_size}")
            return None  # Don't pass incomplete sequences

    # Downsample to 30 frames
        sampled_frames = self.downsample_frames(self.rep_frame_sequence, self.buffer_size)
        features = np.array(sampled_frames, dtype=np.float32).reshape(1, self.buffer_size, -1)
        
        if features.shape[-1] != 132:  # 33 landmarks * 4 features
            logging.error(f"Feature dimension mismatch. Expected 132, got {features.shape[-1]}")
            return None
    ...
    def extract_features(self, landmarks):
        """Convert frame data to feature vector."""
        features = np.array([[lm.x, lm.y, lm.z, lm.visibility] for lm in landmarks], dtype=np.float32).flatten()
        return features
    ...
    def update_frame_buffer(self, landmarks):
        """Update buffer with new frame data."""
        features = self.extract_features(landmarks)
        self.frame_buffer[self.current_frame_idx] = features
        self.current_frame_idx = (self.current_frame_idx + 1) % self.buffer_size
        self.buffer_filled = self.buffer_filled or self.current_frame_idx == 0
        self.rep_frame_sequence.append(features)
            
            
    def start_prediction_loop(self):
        """Start a loop for asynchronous prediction."""
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(self.prediction_consumer())


    async def prediction_consumer(self):
        """Consume data from the queue and run predictions asynchronously."""
        while True:
            features = await asyncio.to_thread(self.prediction_queue.get)
            if features is not None and model is not None:
                prediction = self.model.predict(features, verbose=0)
                await asyncio.to_thread(self.result_queue.put, prediction)
    
    
    def get_prediction(self):
        """Prepare data for prediction and add it to the queue."""
        if len(self.rep_frame_sequence) >= self.buffer_size:
            features = self.process_rep_frames()
            ...
            try:
                ...
                if isinstance(prediction, np.ndarray) and prediction.shape[0] > 1:
                    prediction = prediction[0]  # Take the first element if it's a batch

                return "Good" if prediction > self.prediction_threshold else "Bad"
            except Empty:
                return None
        return None
    
    
    def downsample_frames(self, frames, target_frames=30):
        """Evenly select 30 frames from the entire rep to preserve movement information."""
    
        if len(frames) == target_frames:
            return frames  # Already correct length

        # Select frames evenly across the entire sequence
